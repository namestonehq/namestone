        ...
        for (let i = 1; i <= 3; i++) {
          ...
          await sqlForTests`
            ...
            VALUES 
              (${
                subdomain.id
              }, 'email', ${`${domainType}-${network.name}${i}@example.com`}),
              (${
                subdomain.id
              }, 'url', ${`https://${domainType}-${network.name}${i}.example.com`})
          `;
          ...
          await sqlForTests`
            ...
            VALUES 
              (${
                subdomain.id
              }, 60, ${`0xETH-${domainType}-${network.name}${i}`}),
              (${subdomain.id}, 0, ${`bc1-${domainType}-${network.name}${i}`}),
              (${
                subdomain.id
              }, 2147483785, ${`0xMATIC-${domainType}-${network.name}${i}`})
          `;
        }
      }
    }
    ...
    (networkConfig) => {
      ...
      describe("Domain not specified with API key", () => {
        test("retrieves all subdomains for API key's domains", async () => {
          ...
          responseData.forEach((subdomain) => {
            ...
            expect(subdomain.coin_types["60"]).toMatch(
              new RegExp(`0xETH-primary-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["0"]).toMatch(
              new RegExp(`bc1-primary-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["2147483785"]).toMatch(
              new RegExp(`0xMATIC-primary-${networkConfig.name}`)
            );
          });
        });
        ...
        test("filters by single address", async () => {
          ...
          expect(
            responseData[0].name.startsWith(`primary-${networkConfig.name}`)
          ).toBe(true);
        });
        ...
        test("filters by multiple addresses", async () => {
          ...
          responseData.forEach((subdomain) => {
            expect(
              [TEST_ADDRESSES[1], TEST_ADDRESSES[2]].includes(subdomain.address)
            ).toBe(true);
            expect(
              subdomain.name.startsWith(`primary-${networkConfig.name}`)
            ).toBe(true);
        ...
        describe("Address filtering", () => {
          ...
          test("filters by multiple addresses", async () => {
            ...
            responseData.forEach((subdomain) => {
              expect(
                [TEST_ADDRESSES[1], TEST_ADDRESSES[2]].includes(
                  subdomain.address
                )
              ).toBe(true);
              expect(
        ...
        test("retrieves subdomains from multiple domains when they share API key", async () => {
          ...
          await sqlForTests`
          INSERT INTO api_key (domain_id, key)
          VALUES (${domainIds[networkConfig.name].private}, ${
        ...
        `;
          ...
          responseData.forEach((subdomain) => {
            ...
            const domainType = subdomain.name.startsWith(
              `primary-${networkConfig.name}`
            )
              ? "primary"
              : "private";
            ...
            expect(subdomain.coin_types).toBeDefined();
            expect(subdomain.coin_types["60"]).toMatch(
              new RegExp(`0xETH-${domainType}-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["0"]).toMatch(
              new RegExp(`bc1-${domainType}-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["2147483785"]).toMatch(
              new RegExp(`0xMATIC-${domainType}-${networkConfig.name}`)
            );
          });
      ...
      describe("Domain not specified without API key", () => {
        test("returns only subdomains from public domains", async () => {
          ...
          responseData.forEach((subdomain) => {
            expect(
              subdomain.name.startsWith(`public-${networkConfig.name}`)
            ).toBe(true);
            // Check text records
            expect(subdomain.text_records).toBeDefined();
            expect(subdomain.text_records.email).toBeDefined();
            ...
            expect(subdomain.coin_types).toBeDefined();
            expect(subdomain.coin_types["60"]).toMatch(
              new RegExp(`0xETH-public-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["0"]).toMatch(
              new RegExp(`bc1-public-${networkConfig.name}`)
            );
            expect(subdomain.coin_types["2147483785"]).toMatch(
              new RegExp(`0xMATIC-public-${networkConfig.name}`)
            );
          });
        });
        ...
        test("filters by single address for public domains", async () => {
          ...
          expect(responseData[0].address).toBe(TEST_ADDRESSES[1]);
          expect(
            responseData[0].name.startsWith(`public-${networkConfig.name}`)
          ).toBe(true);
        });
