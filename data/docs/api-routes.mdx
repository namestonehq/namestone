...
> **Service update:** NameStone is shutting down August 3, 2026, and is no longer issuing new API keys. The API documentation remains available for existing users.

Existing domains must use NameStone's resolver:
...
| [set-names](./set-names)     | Set multiple names and text records in a single batch operation (up to 50 names).                      |
